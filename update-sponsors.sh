#!/usr/bin/env bash
# Regenerate the sponsor avatars in src/sponsor.md and in the main hledger
# repo's README.md (between the SPONSOR AVATARS markers) from the Open
# Collective API. Requires curl and jq.
# Run manually (or with "make sponsors") and commit the result in both repos.
...
# The files to update. The second is in the main hledger repo.
FILES="src/sponsor.md ../README.md"
...
git diff --stat src/sponsor.md
git -C .. diff --stat README.md
